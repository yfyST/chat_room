...
class chat_participant {
 public:
  ...
  virtual void deliver(
      const chat_message &msg) = 0;  // participant should deliver message
};
  ...
  void do_read_header() {
    ...
    boost::asio::async_read(
        ...
        boost::asio::buffer(read_msg_.data(),
                            chat_message::header_length),  // data,length
        [this, self](boost::system::error_code ec,
                     std::size_t /*length*/)  // lambada
        {
          if (!ec && read_msg_.decode_header()) {
            do_read_body();  // read body
          } else {
            auto ep_ = socket_.remote_endpoint();
            std::cout << "client : " << ep_.port() << " leave this room"
                      << std::endl;
  ...
  void do_read_body() {
    ...
    boost::asio::async_read(
        ...
        [this, self](boost::system::error_code ec, std::size_t /*length*/) {
          if (!ec) {
            room_.deliver(
                read_msg_);  // deliver将发送消息，刚连接进来的客户将收到历史消息
            do_read_header();  //
          } else {
 ...
 private:
  void do_accept() {
    ...
    acceptor_.async_accept(socket_, [this](boost::system::error_code ec) {
      if (!ec) {
        auto ep_ = socket_.remote_endpoint();
        std::cout << "client : " << ep_.port() << " enter this room"
                  << std::endl;
